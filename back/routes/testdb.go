...
func RegisterTestdbToutes(router fiber.Router) {
	...
	router.Post("/update_test", updateTestDb)
}
...
func updateTestDb(c *fiber.Ctx) error {

	fmt.Println("업데이트 들어옴?!?!")
	type uploadRequest struct {
		Name string `json:"Name"`
		Age  int    `json:"Age"`
	}
	...
	// _, err := db.DB.Exec("UPDATE test_table SET name = ?, age = ?", req.Name, req.Age)
	// if err != nil {
	// 	return c.Status(500).JSON(fiber.Map{"error": err.Error(), "message": "중복된 이름 값이 있습니다."})
	// }
